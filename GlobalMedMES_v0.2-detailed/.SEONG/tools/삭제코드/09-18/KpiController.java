...
public class KpiController {
    ...
    public ResponseEntity<PageResponse<KpiDataListDto>> list(
            ...
            @RequestParam(required = false) String aggregationTypeCode,
        ...
        if (kpiDate != null) {
            result = kpiDataRepo.findByKpiDateAndFilters(kpiDate, equipmentId, processId, itemId, aggregationTypeCode, pageable);
        } else {
            result = kpiDataRepo.findByFilters(equipmentId, processId, itemId, aggregationTypeCode, pageable);
        }

        Page dtoPage = result.map(KpiDataListDto::fromEntity);
