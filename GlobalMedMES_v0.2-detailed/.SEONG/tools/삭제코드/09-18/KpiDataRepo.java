...
public interface KpiDataRepo extends JpaRepository<KpiDataEntity, Long> {
    ...
    @Query("SELECT k FROM KpiDataEntity k JOIN k.aggregationType a WHERE k.kpiDate = :kpiDate " +
            "AND k.equipmentId = :equipmentId AND k.processId = :processId " +
            "AND k.itemId = :itemId AND a.code = :aggregationTypeCode " +
    ...
    Optional<KpiDataEntity> findDailyBatchKpi(
            ...
            @Param("aggregationTypeCode") String aggregationTypeCode,
            @Param("batchGroupKey") String batchGroupKey
    );

    // 실시간 KPI를 위해 추가된 메서드
    @Query("SELECT k FROM KpiDataEntity k JOIN k.aggregationType a WHERE k.workOrderId = :workOrderId " +
            "AND k.equipmentId = :equipmentId AND k.processId = :processId " +
            "AND k.itemId = :itemId AND a.code = :aggregationTypeCode AND k.deleted = false")
    ...
    @Query("SELECT k FROM KpiDataEntity k JOIN k.aggregationType a WHERE " +
            ...
            "(:aggregationTypeCode IS NULL OR a.code = :aggregationTypeCode) AND k.deleted = false")
